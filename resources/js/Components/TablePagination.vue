...
<template>
    <ul class="flex items-center justify-end -space-x-px h-8 text-sm" v-if="typeof arr.data != 'undefined' && arr.data.length > 0">
        <li v-for="(link, index) in arr.links" :key="index">
            ...
            <template v-else-if="index == arr.links.length - 1">
